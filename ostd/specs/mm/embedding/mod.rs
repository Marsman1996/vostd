...
spec fn segment_clone_accounting_transition<'rcu>(
    ...
    &&& forall|paddr: Paddr|
        ...
        (range.start <= paddr < range.end && paddr % PAGE_SIZE == 0) ==> {
            ...
            &&& so_old.inner_perms.ref_count.value() + 1 <= REF_COUNT_MAX
            &&& so_new.inner_perms.ref_count.value() == (so_old.inner_perms.ref_count.value()
                + 1) as u64
            &&& so_new.usage == so_old.usage
            &&& so_new.paths_in_pt == so_old.paths_in_pt
            ...
            ) == 0,
{
    ...
    assert forall|idx: int|
        ...
        if range.start <= paddr < range.end {
            assert(paddr % PAGE_SIZE == 0);
            assert(s_after.regions.slot_owners[idx].inner_perms.ref_count.value() <= REF_COUNT_MAX);
            assert(false);
        } else {
    ...
    ensures
        forall|idx: int|
            ...
            0 <= idx < max_meta_slots() && s_after.regions.slot_owners[idx].usage is Frame
                ...
                != REF_COUNT_UNUSED
                && s_after.regions.slot_owners[idx].inner_perms.ref_count.value()
                != REF_COUNT_UNIQUE ==> handle_count(s_after.frames, idx) > 0
                || s_after.regions.slot_owners[idx].paths_in_pt.len() > 0 || segment_cover_count(
                s_after.segments,
                index_to_frame(idx),
            ) > 0,
